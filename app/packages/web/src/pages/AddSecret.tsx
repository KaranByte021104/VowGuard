...
export function AddSecret() {
  ...
  React.useEffect(() => {
    async function fetchPolicy() {
      try {
        const res = await fetch('http://localhost:3000/policies/active', { credentials: 'include' });
        if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (activePolicy && formData.password) {
      if (formData.password.length < activePolicy.minLength) {
        alert(`Password must be at least ${activePolicy.minLength} characters.`);
        return;
      }
      if (formData.password.length > activePolicy.maxLength) {
        alert(`Password must be no more than ${activePolicy.maxLength} characters.`);
        return;
      }
    ...
    try {
      ...
      // 4. Send to server
      const res = await fetch('http://localhost:3000/secrets', {
        method: 'POST',
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700">
        <div className="grid grid-cols-2 gap-6">
          <div>
            ...
            <select name="templateType" value={formData.templateType} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-gray-50 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
          ...
          <div>
            ...
            <input name="name" required value={formData.name} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-gray-50 dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
          </div>
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Domain / URL</label>
          <input name="domain" value={formData.domain} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-gray-50 dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
        </div>
        ...
        <div className="grid grid-cols-2 gap-6">
          <div>
            ...
            <input name="username" value={formData.username} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 bg-gray-50 dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 flex justify-between">
              <span>Password</span>
              <button type="button" onClick={() => setShowGenerator(!showGenerator)} className="text-primary hover:text-blue-700">Generator</button>
            </label>
        ...
        <div className="flex justify-end gap-4">
          <button type="button" onClick={() => navigate('/secrets')} className="px-4 py-2 border border-gray-300 rounded-md text-gray-700 bg-white hover:bg-gray-50">Cancel</button>
          <button type="submit" disabled={loading || !publicKey} className="px-4 py-2 bg-primary text-white rounded-md hover:bg-blue-700 disabled:opacity-50 flex items-center gap-2">
